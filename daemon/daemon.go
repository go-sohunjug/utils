...
func Background(logFile string, isExit bool, args []string, env []string) (*exec.Cmd, error) {
	// 判断子进程还是父进程
	runIdx++
	...
	// 启动子进程
	cmd, err := startProc(args, env, logFile)
	if err != nil {
...
func (d *Daemon) Run() {
	// 启动一个守护进程后退出
	Background(d.LogFile, true, nil, nil)
	...
	for {
		...
		cmd, err := Background(d.LogFile, false, nil, nil)
		if err != nil { // 启动失败
